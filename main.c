...
/*
 * 设计思路：首先给PS端的DDR3中写入一定数量的数据，然后将DDR3中的数据通过AXI DMA搬运到AXI Stream Data FIFO中，
 * 		接下来将AXI Stream Data FIFO中的数据通过AXI DMA搬回DDR3中。
 * 		需要指定从DDR3中搬出数据的起始地址、数据搬回DDR3的存储地址以及搬运的字节长度。
 * */
 ...
 #define MAX_PKT_LEN             0x100    //发送包长度 256字节
